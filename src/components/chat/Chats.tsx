...
const isAllowedChatAttachment = (file: any) => {
  if (!file) {
    return false;
  }
  ...
  return allowedChatAttachmentExtensions.has(extension as any);
};
...
const isVideoAttachment = (attachment: any = {}) => {
  if (!attachment) {
    return false;
  }
...
const isVoiceAttachment = (attachment: any = {}) =>
...
const buildDocumentViewerUrl = (attachment: any = {}) => {
...
const buildAttachmentDownloadUrl = (attachment: any = {}) => {
  const sourceUrl = String(attachment?.url || "").trim();
  if (!sourceUrl) {
    return "";
  }
...
const getChatLastMessagePreview = (textMessage = "", attachment: any = null) => {
...
const getReplyAttachmentFallback = (replyTo: any = {}) => {
...
const getReplyPreviewText = (replyTo: any = {}) => {
...
function Chats() {
  ...
  const [chattingWith, setChattingWith] = useState<any>(null);
  ...
  const recordingChunksRef = useRef<any>([]);
  ...
  useEffect(() => {
    if (socket && chatId) {
      ...
      socket.on("typing_indicator", ({ username, isTyping }: any) => {
        setIsTyping(isTyping);
        setWhoIsTyping(username);
      });

      socket.on("message_sent", (confirmation: any) => {
  ...
  const handleChatSelect = (selectedChat: any) => {
  ...
  const handleAttachmentSelection = (event: any) => {
  ...
  const uploadChatAttachment = async (attachmentPayload: any) => {
  ...
  const handleReplySelection = (sourceMessage: any) => {
                ...
                {showChatMenu ? (
                  ...
                  >
                    ...
                    <div className="mt-1 space-y-1">
                      {Object.entries(CHAT_THEMES).map(([themeKey, theme]: any) => (
                        <button
                          key={themeKey}
                          type="button"
                          onClick={() => {
