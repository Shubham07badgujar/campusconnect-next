...
const isImageType = (media: any = {}) => String(media?.type || "") === "image";
const isVideoType = (media: any = {}) => String(media?.type || "") === "video";

const buildMapEmbedUrl = (lat: any, lng: any) => {
...
function FixItBoard({ role = "student", displayName = "" }: any) {
  const [user] = useAuthState(auth);

  const [description, setDescription] = useState("");
  const [locationText, setLocationText] = useState("");
  const [locationPoint, setLocationPoint] = useState<any>(null);
  ...
  const uploadMedia = async (file: any, type: any) => {
  ...
  const submitComplaint = async (event: any) => {
  ...
  const toggleUpvote = async (complaint: any) => {
    ...
    try {
      ...
      await runTransaction(firestore, async (transaction) => {
        ...
        const nextVoters = alreadyUpvoted
          ? currentVoters.filter((uid: any) => uid !== user.uid)
  ...
  const markResolved = async (complaint: any) => {
