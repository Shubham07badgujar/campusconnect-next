...
function AcademicCalendar() {
  ...
  const [parsedOcrEntries, setParsedOcrEntries] = useState<any[]>([]);
  const [parsedOcrHolidayEntries, setParsedOcrHolidayEntries] = useState<any[]>(
    [],
  );
  const [calendarPdfFile, setCalendarPdfFile] = useState<any>(null);
  ...
  const [academicCalendarPdfs, setAcademicCalendarPdfs] = useState<any[]>([]);
  ...
  const handleOcrFileChange = (event: any) => {
  ...
  const handleCalendarPdfFileChange = (event: any) => {
    const selected = event.target.files?.[0] || null;
    if (!selected) {
  ...
  const updateParsedOcrEntry = (index: any, field: any, value: any) => {
  ...
  const removeParsedOcrEntry = (index: any) => {
    setParsedOcrEntries((prev) =>
  ...
  const updateParsedHolidayEntry = (index: any, field: any, value: any) => {
  ...
  const removeParsedHolidayEntry = (index: any) => {
    setParsedOcrHolidayEntries((prev) =>
      prev.filter((_, rowIndex) => rowIndex !== index),
    );
  };
  ...
  const formatShortDate = (isoDate: any) => {
  ...
  const getEventDateDisplay = (item: any) => {
