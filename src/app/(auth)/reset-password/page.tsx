...
function ResetPassword() {
  ...
  useEffect(() => {
    const prefilledLoginId = String(searchParams.get("loginId") || "").trim();
  ...
  const handleVerifyOtp = async (e: any) => {
    ...
    try {
      ...
      router.push(
        `/reset-password-confirm?loginId=${encodeURIComponent(
          String(data.loginId || loginId).trim(),
        )}&maskedEmail=${encodeURIComponent(
          String(data.maskedEmail || maskedEmail),
        )}&resetToken=${encodeURIComponent(String(data.resetToken || ""))}`,
      );
    } catch (verifyError) {
